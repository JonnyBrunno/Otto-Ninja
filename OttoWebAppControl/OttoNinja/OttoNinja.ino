...
//
//  IMPORTANTE: a detecção usa uma INTERRUPÇÃO de hardware (attachInterrupt),
//  não um simples polling (digitalRead a cada volta do loop). Isso é essencial
//  porque o Sentinela (e outras rotinas) usam várias chamadas de delay() em
//  sequência - se a detecção do clique dependesse só de checar o pino a cada
//  volta do loop(), um clique dado bem no meio de um desses trechos bloqueantes
//  simplesmente não seria percebido (era esse o bug da versão anterior: o
//  primeiro clique, com o robô parado/ocioso, era percebido rápido, mas os
//  cliques seguintes, com o Sentinela em movimento, muitas vezes se perdiam).
//  A interrupção é atendida pelo hardware na hora, não importa o que o loop()
//  esteja fazendo (mesmo dentro de um delay()).
// =====================================================================================
volatile int buttonClickCount = 0;
volatile unsigned long lastButtonClickTime = 0;
const unsigned long BUTTON_ISR_DEBOUNCE_MS = 50;   // ignora cliques mecânicos "picotados" mais próximos que isso
...
// Executada pelo hardware assim que o botão é pressionado (borda de descida),
// não importa o que o resto do código esteja fazendo no momento.
void IRAM_ATTR handleButtonInterrupt() {
  unsigned long now = millis();
  if (now - lastButtonClickTime > BUTTON_ISR_DEBOUNCE_MS) {
    buttonClickCount++;
  }
...
// Decide o que fazer com os cliques já contados pela interrupção (handleButtonInterrupt):
// espera passar a janela de clique duplo sem um novo clique pra então decidir
// entre "1 clique" (troca de função) e "2 cliques" (desliga).
void CheckButtonClicks() {
  // Copia os valores voláteis rapidinho, com interrupções desligadas, pra
  // evitar que a ISR mude o valor bem no meio da leitura.
  noInterrupts();
  int count = buttonClickCount;
  unsigned long lastClick = lastButtonClickTime;
  interrupts();

  if (count > 0 && (millis() - lastClick) > DOUBLE_CLICK_WINDOW_MS) {
    noInterrupts();
    buttonClickCount = 0;
    interrupts();
...
void setup() {
  ...
  // NOVO: interrupção de hardware no clique do botão - garante que nenhum
  // clique se perca mesmo durante os trechos bloqueantes (delay()) do Sentinela
  // e de outras rotinas de movimento.
  attachInterrupt(digitalPinToInterrupt(SHUTDOWN_PIN), handleButtonInterrupt, FALLING);
  digitalWrite(LED_PIN, LOW);
